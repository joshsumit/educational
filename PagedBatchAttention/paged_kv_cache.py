...
class MultiSequencePagedKVCache:
    ...
    def _ensure_writable_block(self, sequence_id: str):
        ...
        if not state.sequence_blocks or not state.sequence_blocks[-1].has_space():
            ...
            state.sequence_blocks.append(block)
            subsection("SEQUENCE RECEIVED A NEW WRITABLE PHYSICAL BLOCK")
            print(f"Sequence id: {sequence_id}")
            print(f"New physical block assigned: {block.block_id}")
    ...
    def append_token(self, sequence_id: str, k_token: np.ndarray, v_token: np.ndarray):
        ...
        record = {"token_index": state.total_tokens, "block_id": block.block_id, "slot": slot}
        ...
        print_tensor("K token being written", k_token, f"One token's K for sequence {sequence_id}", dim_names=["H", "Dk"])
        print_tensor("V token being written", v_token, f"One token's V for sequence {sequence_id}", dim_names=["H", "Dk"])
    ...
    def dump_physical_blocks(self, sequence_id: str = None):
        targets = [(sequence_id, self.sequences[sequence_id])] if sequence_id is not None else list(self.sequences.items())
        subsection("PHYSICAL BLOCK CONTENTS")
        for seq_id, state in targets:
            print(f"\nSequence id: {seq_id}")
    ...
    def gather_active_kv(self, sequence_id: str):
        ...
        print_tensor("K gathered from all blocks", K_gathered, f"All cached keys for sequence {sequence_id}", dim_names=["H", "T", "Dk"])
        print_tensor("V gathered from all blocks", V_gathered, f"All cached values for sequence {sequence_id}", dim_names=["H", "T", "Dk"])
