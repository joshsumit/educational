...
def scaled_dot_product_attention(Q, K, V, use_causal_mask: bool, phase_name: str):
    ...
    print_matrix_multiply("MatMul for raw attention scores", left_shape=Q.shape, right_shape=K_t.shape, out_shape=(B, H, S, T), explanation="Each query compares itself to every key in its own context")
    ...
    if use_causal_mask:
        ...
        print_tensor("Raw attention scores after causal mask", scores, "Future positions receive large negative values", dim_names=["B", "H", "S", "T"])
    else:
        print("\nNo causal mask applied in this call. This is expected during single-token decode.")
    weights = softmax(scores, axis=-1)
    print_tensor("Attention weights after softmax", weights, "Probability distribution over context tokens", dim_names=["B", "H", "S", "T"])
    print_matrix_multiply("MatMul for weighted value aggregation", left_shape=weights.shape, right_shape=V.shape, out_shape=(B, H, S, Dk), explanation="Attention weights produce a weighted sum of V for each head")
